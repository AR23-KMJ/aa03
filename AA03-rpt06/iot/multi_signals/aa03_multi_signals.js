...
parser.on("data", (data) => {
  ...
  firstcommadix = readData.indexOf(",");
  secondcommadix = readData.indexOf(",", firstcommadix + 1);
  if (firstcommadix > 0) {
    temp = readData.substring(0, firstcommadix);
    humi = readData.substring(firstcommadix + 1, secondcommadix);
    lux = readData.substring(secondcommadix + 1);
    readData = "";
  ...
  mdata[2] = humi;
  mdata[3] = lux
  console.log("AA03," + mdata.toString());
